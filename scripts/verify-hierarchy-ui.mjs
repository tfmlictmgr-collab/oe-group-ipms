...
let region, project, prop;
{
  region = (await mkNode(null, "region", `PROBE-UI-Region-${S}`)).data;
  project = (await mkNode(region.id, "project", `PROBE-UI-Project-${S}`)).data;

  const { error: e1 } = await svc.rpc("retire_org_node", { p_node_id: region.id });
  e1 ? ok("a region with a live project cannot be retired") : bad("A REGION WITH A LIVE CHILD WAS RETIRED");

  const location = (await mkNode(project.id, "location", `PROBE-UI-Location-${S}`)).data;
  const site = (await mkNode(location.id, "site", `PROBE-UI-Site-${S}`)).data;
  ...
  const { error: e5 } = await svc.rpc("retire_org_node", { p_node_id: location.id });
  e5 ? bad("a location whose only child is retired still could not be retired")
     : ok("a location whose child is now retired can itself be retired");
}
...
console.log("\nB. Retiring is gated on hierarchy.write, same as every other write here");
{
  ...
  const { error } = await c.rpc("retire_org_node", { p_node_id: project.id });
...
console.log("\nC. org_nodes_overview's counts are correct, not just present");
{
  ...
  const projA = (await mkNode(region2.id, "project", `PROBE-UI-ProjA-${S}`)).data;
  await mkNode(region2.id, "project", `PROBE-UI-ProjB-${S}`);
  const locA = (await mkNode(projA.id, "location", `PROBE-UI-LocA-${S}`)).data;
  const siteA = (await mkNode(locA.id, "site", `PROBE-UI-SiteA-${S}`)).data;
  ...
  const { data: rows, error } = await svc.from("org_nodes_overview")
    ...
    .in("id", [region2.id, projA.id, siteA.id]);
  if (error) { bad(`could not read org_nodes_overview — ${error.message.slice(0, 70)}`); }
  else {
    const byId = Object.fromEntries(rows.map((r) => [r.id, r]));
    byId[region2.id]?.child_count === 2
      ? ok("the region's child_count is exactly its two projects")
      : bad(`region child_count was ${byId[region2.id]?.child_count}`);
    byId[projA.id]?.child_count === 1
      ? ok("the project's child_count is its one location")
      : bad(`project child_count was ${byId[projA.id]?.child_count}`);
...
console.log("\nD. property_summary carries the property's place in the tree");
{
  ...
  const proj3 = (await mkNode(region3.id, "project", names.project)).data;
  const loc3 = (await mkNode(proj3.id, "location", names.location)).data;
  const site3 = (await mkNode(loc3.id, "site", names.site)).data;
  ...
  byId[filed.id]?.node_path === [names.region, names.project, names.location, names.site].join(" / ")
