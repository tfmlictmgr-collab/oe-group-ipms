...
**Cut at `374c2a4`, the merge of PR #53 into `main` (23 Sept 2026).** Stage 0
was run on `fe319be`, the PR head already merged with `main` at `4af81db`.
`git diff fe319be 374c2a4 -- . ':!docs'` is **empty**, so every byte outside
`docs/` is the tree that was verified. The only difference is PR #54
(`docs/BACKUP_AND_RESTORE.md`), which landed between the run and the merge.
This file and the run log reached `main` in a follow-up PR, because #53 was
merged before they were pushed. They are records of the tag, not part of it.

A tag push from the build session is refused (HTTP 403), so a person creates it:

```
git fetch origin
git tag -a v1.0.0-rc4 374c2a4 -F docs/verify-runs/rc4-tag-message.md
