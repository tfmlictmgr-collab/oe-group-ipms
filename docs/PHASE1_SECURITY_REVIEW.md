...
|----|---------|-----|-------|
| S1 | Live webhook auth *default-skips* when the secret is unset → forged POSTs can create tickets, spend Anthropic tokens, and make our verified number reply to arbitrary recipients | Auth now **fails closed in production** (Vercel `production` / `NODE_ENV=production`) when the secret is missing; skip only outside prod. Telegram token compare made constant-time. | `lib/webhook-security.ts` |
...
1. **S1 ops step — set the webhook secrets in Vercel:** `WHATSAPP_APP_SECRET` and
   `TELEGRAM_WEBHOOK_SECRET`. After this branch deploys, prod **fails closed**
   without them (the live number's webhook will reject until they're set). Until
   deploy, consider setting them now on `main`'s deployment to close the live
   window immediately.
