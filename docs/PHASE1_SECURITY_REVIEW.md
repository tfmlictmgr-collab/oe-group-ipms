...
> **Status update (2026-07-24, PC1): all of this is now merged into `main`.** The
> `phase-1-hardening` branch was collapsed into `main` and deleted rather than kept
> alive, so there is **one baseline** for Phase 1. Migration `0010` ships in
> `supabase/migrations/` and is applied to the POC DB — deliberately, because
> `scripts/migrate.mjs` is ledger-driven and would have applied it on the next
> migration run anyway; leaving it "pending" was a deferred surprise, not a safe
> state. Compatibility was verified against the live schema first (see
> `RECONCILED_ROADMAP.md` → "Live verification of the demo"). The frozen
> pre-hardening demo remains available at tag `poc-demo-v1`.
