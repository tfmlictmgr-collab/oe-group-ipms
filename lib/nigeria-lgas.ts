// The 774 local government areas, keyed by the state they sit in.
//
// This is the LGA twin of `0186`'s reasoning about locations, one level down:
// the application form asked for "Local government area" as FREE TEXT directly
// beneath a state DROPDOWN, so one applicant writes "Ikeja", the next "IKEJA"
// and the third "Ikeja LGA" — three values a sibling-name constraint cannot
// catch, because they are genuinely different strings. An offered list is what
// stops the spellings.
//
// ⚠️ It is offerable precisely BECAUSE it is closed. Nigeria has exactly 774
// LGAs, fixed by the constitution, and a partial list would be worse than none:
// decision 20's whole finding was that 25 seeded cities were "a sample, not a
// set", so an applicant from an unlisted one had to invent a value and the
// spellings came back. Every state below carries its complete set.
//
// 📌 The keys must match `NIGERIAN_STATES` in `lib/application-form.ts`
// EXACTLY, including "FCT — Abuja" with its em dash. A key that does not match
// yields an empty dropdown rather than an error, so `lgaKeysMatchStates()`
// below exists to make that a test rather than a discovery.
//
// Source: the constitutional list as published by INEC and the NBS. Worth a
// check against a current authoritative copy before go-live — this is
// transcribed data, and a missing LGA is silent to everyone except the
// applicant who lives in it.
...
export const LGA_BY_STATE: Record<string, string[]> = {
  ...
  Lagos: [
    ...
    "Lagos Island", "Lagos Mainland", "Mushin", "Ojo", "Oshodi-Isolo", "Somolu",
    "Surulere",
  ],
  Nasarawa: [
  ...
  Osun: [
    "Atakumosa East", "Atakumosa West", "Aiyedaade", "Aiyedire", "Boluwaduro", "Boripe",
...
/** Every LGA offered for one state, or none where the state is unknown. */
export function lgasFor(state: string | null | undefined): string[] {
  if (!state) return [];
  return LGA_BY_STATE[state] ?? [];
}

/**
 * Whether an LGA belongs to the state chosen.
 *
 * ⚠️ Used to CLEAR a stale LGA when the state changes, which is the bug a
 * dependent dropdown creates if nobody writes this: pick Lagos, pick Ikeja,
 * change your mind to Kano, and the form still holds Ikeja — a combination that
 * exists nowhere, submitted by someone who watched the state field change and
 * had no reason to look back at the one below it.
 */
export function lgaBelongsTo(state: string | null | undefined, lga: string | null | undefined): boolean {
  if (!lga) return true;
  return lgasFor(state).includes(lga);
}

/** The total, so a miscount in the table above is a failed test and not a silence. */
export const TOTAL_LGAS = Object.values(LGA_BY_STATE).reduce((n, l) => n + l.length, 0);
